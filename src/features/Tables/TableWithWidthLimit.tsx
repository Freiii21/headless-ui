...
export const TableWithWidthLimit: FC<Props> = ({ title, users }) => {
							...
							return (
								<StyledTr key={row.id}>
									{row.getVisibleCells().map((cell) => {
										return (
											<StyledTd key={cell.id} size={cell.column.getSize()} isPinned={cell.column.getCanPin()}>
...
const StyledTh = styled.th<{ size: number; isPinned: boolean }>`
	...
	left: ${({ isPinned }) => isPinned && "0"};
	z-index: ${({ isPinned }) => isPinned && "1"};
`;
...
const StyledTd = styled.td<{ size: number; isPinned: boolean }>`
	min-width: 5rem;
	max-width: ${({ size }) => `${size}px`};
	overflow: hidden;
	padding: 1rem 2rem;
	position: ${({ isPinned }) => isPinned && "sticky"};
	left: ${({ isPinned }) => isPinned && "0"};
